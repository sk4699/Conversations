...
def build_players_list(args) -> list[type[Player]]:
    ...
    return [RandomPlayer] * args.players["pr"] + [Player1] * args.players["p1"] + [RandomPausePlayer] * args.players["prp"]

# -------- Run one game for one weight vector --------
def run_once_with_weights(args, weights: Tuple[float, float, float, float, float], seed_offset: int) -> Dict[str, Any]:
...
def sweep_and_export(args):
    ...
    df = df.sort_values(["shared_total", "player1_individual"], ascending=False).reset_index(drop=True)
